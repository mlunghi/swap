...
describe("swap", () => {
  ...
  it("Initiates token program", async () => {
    ...
    let [balances, bump_balances] = await PublicKey.findProgramAddress([anchor.utils.bytes.utf8.encode("my-token-seed")], program.programId)
    //  pda for authority
    let [authority, bump_authority] = await PublicKey.findProgramAddress([anchor.utils.bytes.utf8.encode("open-orders-account")], program.programId)
